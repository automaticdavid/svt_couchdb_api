...
class Utils:
    ...
    def expander(self, j, viprsource, links):
        ...
        # Standalone strings might also be links
            ...
            if j != 'id' and isinstance(j, str) and j.startswith('urn:'):
    ...
    def cleanKeys(self, r):
            ...
            for row in j['rows']:
                ...
                svt_cdb_key = row['key']
                cdb_row = {
                    'svt_cdb_key': svt_cdb_key,
                    'svt_cdb_value': svt_cdb_value
                }
                r.append(cdb_row)
            d = json.dumps(r)
            return(d)
    ...
    def jsonify(self, res, caller, rows):
        ...
        for row in rows:
            ...
            if 'info' not in res:
                ...
                res['info']['svt_collect'] != collect and
                res['info']['svt_client'] != client
            ):
                code = 99
            ...
            if (
                ...
                if marker not in leaf:
                    ...
                    leaf[marker] != 'svt_no_data' and
                    value != 'svt_no_data' and
                    value != leaf[marker]
                ):
